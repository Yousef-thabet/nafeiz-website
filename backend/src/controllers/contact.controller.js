...
const { sendLeadToEspoCRM } = require('../services/crm.service');
...
const createContact = async (req, res, next) => {
  try {
    ...
    const normalizedPhone =
      parsed.data.phone ||
      [parsed.data.dialCode, parsed.data.phoneNumber]
        .filter(Boolean)
        .join(' ')
        .trim();
    ...
    const data = await prisma.contact.create({
      data: {
        ...
        estimatedOrderQuantity:
          parsed.data.estimatedOrderQuantity || null,
    ...
    const crmResult = await sendLeadToEspoCRM(data);

    if (!crmResult.ok) {
      console.error(
        'EspoCRM sync failed:',
        crmResult.error || crmResult.reason
      );
    } else {
      console.log(
        'EspoCRM Lead created successfully:',
        crmResult.id || crmResult.data?.id
      );
    }
...
const assignContact = async (req, res, next) => {
  try {
    ...
    if (!employee) {
      return sendError(res, 'Active employee not found', [], 404);
    }
